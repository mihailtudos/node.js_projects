...
const forecast = (lat, lon, cb) => {
	...
	request({url, json: true}, (error, {body: {  error: locationError, current: weather, location}}) => {
		if (error) {
			...
			cb(undefined, `${weather.weather_descriptions[0]} in ${location.region}, ${location.country}. It is currently ${weather.temperature} degrees out. There is a ${weather.precip}% chance of raining`);
		}
	})
}
